...
# 当前阶段
查询范围扩大后的脚本准备阶段。lyx 已确认最短停留改为 7 天，并要求开口程按全部非英国欧洲城市两两有序组合查询；当前已完成代码修改和离线校验，正在评估全量执行耗时。
...
- 往返日期规则确定：去程出发日期和回程出发日期都在 `2026-09-25` 至 `2026-10-10` 内，停留天数至少 7 天，共 45 个日期组合；21 个非英国目的地同城往返共 945 组。
...
- lyx 最新频率要求：`30-60s` 可能触发风控，当前正式运行参数使用 `180-300s` 随机等待。
...
- 开口程会生成同一城市对的 group CSV，并按 `往返含税价` 升序排序。
...
# 下一步动作
- 7 天口径已离线校验：`2026-10-01` 出发包含 `2026-10-08`、`2026-10-09`、`2026-10-10` 返程。
- 全量开口程范围已离线校验：21 个非英国目的地生成 420 个有序城市对，合计 18,900 个日期组合查询。
- 同城往返范围为 945 个查询；若同城往返和全量开口程都跑，总查询数为 19,845。
- 以当前 `180-300s` 随机等待估算，仅等待时间约 55 天；计入页面加载、技术超时、登录/安全验证等待后，全量可能需要约 63-80 天。
- 下一步需由 lyx 决定是否接受该执行时长并启动全量开口程/同城往返长任务；如遇验证码/风控，只等待 lyx 手动完成验证，不代点验证码。

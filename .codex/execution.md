...
- 13:14 已完成 5 组后尝试切到后台进程，当前工具环境会清理后台子进程，未能持久运行；已重新用前台会话继续。
- 14:26 巴黎航线 28 个日期组合均已写入同一个 `上海-巴黎.csv`；其中 25 行有有效 `往返含税价`，3 行为技术超时产生的空价格失败记录，后续补跑会覆盖对应日期组合。
- 巴黎当前最低有效价为 `5961`，日期组合为 `2026-09-26 -> 2026-10-08` 与 `2026-09-27 -> 2026-10-08`。
- 14:32 已进入下一目的地 `上海-法兰克福`，首组 `2026-09-25 -> 2026-10-04` 成功写入 `上海-法兰克福.csv`。
- 14:45 截图确认旧 cookies 触发携程登录弹窗；已修复 runner：检测到登录弹窗时等待 lyx 手动登录，保存 cookies 后重跑当前组合，不再直接写空价格失败行。
- 15:25 登录等待流程实际跑通：保存 18 条新携程 cookies，并自动重跑 `上海-巴黎 2026-09-30 -> 2026-10-10`。
- 15:27 巴黎航线已补齐，`上海-巴黎.csv` 共 28 行，28 行都有有效 `往返含税价`，空价格为 0。
- 已修复返程页判断：去程点击后不再只等旧版 `.segment_tab.active`，也支持新版页面正文/顶部步骤中的“选择返程”。
- 已修复日期弹层遮挡搜索按钮：返程日期选择后主动关闭日期面板再点击搜索。
- 15:34 与 15:38 法兰克福连续两组成功，验证返程页判断修复有效。

## 2026-05-14 最短停留调整为 8 天
- lyx 明确要求最短停留改为 8 天。
- 已将 `ctrip_flights_scraper_V3.py` 中 `min_stay_days` 从 9 改为 8。
- 离线校验：
  - `min_stay_days = 8`
  - 日期组合数从 28 变为 36。
  - `2026-10-01` 出发包含 `2026-10-09`、`2026-10-10` 返程。
  - `2026-10-01 -> 2026-10-08` 不包含，因为停留 7 天。
- 验证：`.venv/bin/python -m py_compile ctrip_flights_scraper_V3.py run_flight_job.py` 通过。

## 2026-05-14 按 8 天口径恢复执行
- lyx 确认开始执行后，已启动：
  `.venv/bin/python run_flight_job.py --all-cities --run-day 2026-05-14 --min-interval 180 --max-interval 300 --max-wait 90 --max-search-wait 180 --max-consecutive-failures 5 --manual-verification-wait 1800`
- 本轮任务启动时待执行 715 组；脚本按 180-300 秒随机间隔低频查询。
- 已确认新增 8 天组合开始执行：`上海-巴黎 2026-09-25 -> 2026-10-03` 成功写入 `上海-巴黎.csv`。
- `2026-10-01 -> 2026-10-09` 已进入任务队列；首次执行因首页超时写入空价失败行，后续会补跑覆盖。
- 18:14 状态：本轮成功 17 组、失败 4 组，无登录等待、无安全验证等待。
- 当前路线文件概况：
  - `上海-巴黎.csv`：36 行，34 行有效价格，2 行空价格待补，当前最低价 `5961`。
  - `上海-法兰克福.csv`：26 行，24 行有效价格，2 行空价格待补，当前最低价 `5374`。

## 2026-05-14 最短停留调整为 7 天并支持全量开口程
- lyx 明确要求最短停留改为 7 天，以包含 `2026-10-01 -> 2026-10-08/09/10`。
- 已将 `ctrip_flights_scraper_V3.py` 中 `min_stay_days` 从 8 改为 7。
- 已新增 `run_flight_job.py --all-open-jaw`，按脚本中的 21 个非英国欧洲目的地生成全部有序开口程城市对，排除进出欧洲同城。
- 开口程结果改为直接写入同一城市对的航线级 CSV，例如 `上海-巴黎__米兰-上海.csv`，并按 `往返含税价` 升序排序；不再为每个日期组合生成过程 CSV。
- 离线校验：
  - `min_stay_days = 7`
  - 日期组合数为 45。
  - `2026-10-01` 出发包含 `2026-10-08`、`2026-10-09`、`2026-10-10` 返程。
  - 21 个城市生成 420 个有序开口程城市对。
  - 同城往返查询数为 945；全量开口程查询数为 18,900；两者合计 19,845。
- 验证：
  - `.venv/bin/python -m py_compile ctrip_flights_scraper_V3.py run_flight_job.py` 通过。
  - `.venv/bin/python run_flight_job.py --help` 正常展示 `--all-open-jaw`。
- 以当前 `180-300s` 随机等待估算，仅等待时间约 55 天；计入页面加载、技术超时、登录/安全验证等待后，全量同城往返加开口程可能需要约 63-80 天。
